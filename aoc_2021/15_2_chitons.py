...
def multiply_grid(grid, many):
    ...
    for i in range(many):
        for row in grid:
            ...
            for j in range(many):
                for elt in row:
                    new_row += str((int(elt) + j + i - 1) % 9 + 1)
            new_grid.append(list(map(int, list(new_row))))
    return new_grid
...
def main(my_file):
    ...
    new_grid = multiply_grid(grid, 5)
    size = len(new_grid) - 1
    distances, predecessors = dijkstra(new_grid)
