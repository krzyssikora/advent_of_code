...
def main(my_file):
    ...
    im_starting = True
    final_number = None
    length = len(snailfish_numbers)
    for i in range(length):
        for j in range(length):
            if i != j:
                number_1 = string_to_snailfish(snailfish_string(snailfish_numbers[i]))
                number_2 = string_to_snailfish(snailfish_string(snailfish_numbers[j]))
                magnitude = snailfish_magnitude(simplify(snailfish_add(number_1, number_2)))
                if magnitude > max_magnitude:
                    max_magnitude = magnitude
                    best = i, j
    best_s = snailfish_numbers[best[0]], snailfish_numbers[best[1]]
    print(max_magnitude)
    print(best[0], snailfish_string(best_s[0]))
    print(best[1], snailfish_string(best_s[1]))
